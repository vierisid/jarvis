...
func fillCircle(pixels []uint32, cx, cy, r float64, col uint32) {
	...
	if x1 > pebbleWindowSizePx {
		x1 = pebbleWindowSizePx
	}
	if y1 > pebbleWindowSizePx {
		y1 = pebbleWindowSizePx
	}
	colA := uint8(col >> 24)
	for py := y0; py < y1; py++ {
		for px := x0; px < x1; px++ {
			...
			pixels[py*pebbleWindowSizePx+px] = blendOver(pixels[py*pebbleWindowSizePx+px], src)
		}
	}
}
...
func (s *pebbleServiceWindows) drawIdle(pixels []uint32) {
	cx := float64(pebbleWindowSizePx / 2)
	cy := float64(pebbleWindowSizePx / 2)
...
func fillRoundedRect(pixels []uint32, x0, y0, x1, y1, r float64, col uint32) {
	...
	if ix1 > pebbleWindowSizePx {
		ix1 = pebbleWindowSizePx
	}
	if iy1 > pebbleWindowSizePx {
		iy1 = pebbleWindowSizePx
	}
	r8 := uint8((col >> 16) & 0xFF)
	g8 := uint8((col >> 8) & 0xFF)
	b8 := uint8(col & 0xFF)
