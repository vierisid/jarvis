...
- **OpenRouter**: most routed upstreams (OpenAI, Gemini 2.5, DeepSeek, Groq, Grok, Moonshot) cache automatically. For upstreams that need explicit breakpoints (`anthropic/*`, `qwen/*` models) Jarvis sends OpenRouter's top-level `cache_control: {type: "ephemeral"}` and OpenRouter places the breakpoints itself; it also sticky-routes follow-up requests to the same upstream endpoint to maximize hit rates. Cached reads and writes come back in `prompt_tokens_details` and are surfaced in telemetry (both chat and streaming).
