...
Awareness start and stop requests are serialized. New capture events are rejected immediately on a stop request. Re-enabling while delivery shutdown is pending resumes the service after it drains, unless a later disable or explicit daemon shutdown cancels that request.
