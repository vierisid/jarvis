...
### P3.7 — expansion batch 7 (87 → 100 templates) 🎯 THE 100 GOAL
◐ IN PROGRESS 2026-07-16: notioncal (calendar; empty kw — gcalendar owns calendar vocab, "notion" homonym), miro (whiteboard, canvas-honesty like figma), zendesk (support tickets read + gated), hubspot (CRM read + gated), gbooks (book search/read read-only; kw disjoint from goodreads), gfinance (market data read-only), duolingo (progress read; lessons gamified/out-of-scope), bandcamp (music; purchase gate), soundcloud (audio stream; kw disjoint from spotify), zapier (automation read; edits gated), coda (docs block-editor; "coda" homonym), devto (dev blog read + gated publish; kw disjoint from medium), grafana (observability read-only like ganalytics). Same conventions.
