...
- ☐ **Matcher hardening** (`src/vault/webapp-templates.ts`): word-boundary matching for app names (kills "notion"/"slack" English-word hits); score matches and inject only the best (or cap total injected KB); dedupe by domain.
- ☐ **Template lint harness** (new, e.g. `scripts/lint-webapp-templates.ts` + tests): schema check; executability lint (flag hover/Ctrl+/Tab/Escape/arrow-key steps unless the tool surface supports them); banned-tool-vs-used-tool consistency; keyword genericity check against a corpus of non-webapp prompts; per-template size budget (target ≤12KB); forbid positional selectors on destructive actions.
