...
- ◐ **Live-capture pass** (2026-07-16): split into two tiers.
  **Tier 1 — anonymous-readable (IN PROGRESS, local headless Chromium via `scripts/capture-snapshot.ts`):** ~25 templates whose sites browse without login (hackernews, wikipedia, archive, arxiv, huggingface, npm, pypi, dockerhub, imdb, reddit, medium, translate, gnews, gmaps, gflights, youtube, twitch, booking, tripadvisor, amazon, ebay, etsy, bluesky, pinterest, threads). Verified claims get their [LIVE-VERIFY] marker removed; wrong claims get FIXED; login-gated claims keep markers. Captures are strictly read-only (navigate+snapshot; never submit/post/login). Bot-walled sites recorded honestly.
