...
### Phase 1 — Adopt-vs-build gate (de-risk spike) · ☐ · ~2 weeks · **GATE THE PROGRAM ON THIS**
Run two desktop spikes + one browser spike against the same acceptance test, then decide.
- ☐ **Spike A (adopt):** cua-driver as sidecar desktop engine — spawn as MCP-over-stdio subprocess (exactly Hermes' integration); map its snapshot/act surface into `SemanticSurface`/`ui_act`. Measure: latency, tree quality on Slack/Outlook/Notepad, background-input reliability, packaging weight, license hygiene.
- ☐ **Spike B (build):** enrich our own `uiaInspect` to emit `AutomationId`/`RuntimeId`/ancestry/patterns → `SemanticRef` (v1 Phase-0 scope), staying on the COM thread.
- ☐ **Browser spike:** CDP `Accessibility.getFullAXTree` + `DOM` provider with **attach-first strategy**: ① Chrome 144+ auto-connect ② extension/native-messaging mode ③ jarvis-managed Chrome (isolated profile) as last resort. Never debug-port-relaunch the user's default profile (dead since M136). Evaluate vendoring chrome-devtools-mcp/Playwright MCP vs keeping bespoke Go CDP.
...
- **Exit criterion:** ≥90% step success on both flows, re-resolution ≥95%, ≥8× token reduction — via whichever option wins. Decision recorded with the matrix (latency / coverage / maintenance / license / packaging).
