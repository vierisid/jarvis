...
- ☑ `Skill` store (`src/vault/skills.ts` + `skills` table), `run_skill` executor (`src/skills/runtime.ts`) over the structural path with per-step postcondition verify + self-heal (re-resolve→retry); `webapp_templates` coexists (dual-run) and `matchSkills` is now URL/active-tab/process-aware. *(2026-07-30 rebase note: main's #275 replaced message-time template injection with URL-at-browse-time delivery — the skill index now injects via its own `skillIndex` prompt channel and message-time template matching no longer exists.)* `run_skill`/`manage_skills` tools. Full unit coverage.
