...
- **Context-Aware Defaults**: 
    - **Local Mode**: Instantly defaults the Base URL to `http://localhost:11434` and hides the API Key field.
    - **Cloud Mode**: Resets the field to a clean `https://` state and enables the API Key field for authenticated proxies.
- **Authentication Support**: Full support for Bearer token authorization in Cloud mode, enabling connections to OpenAI-compatible Ollama proxies.
- **Editable Persistence**: The URL field remains fully editable regardless of the toggle state.
