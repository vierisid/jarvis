...
export async function runOnboard(): Promise<void> {
  ...
  if (provider === 'anthropic') {
    ...
    const existing = config.llm.xai?.api_key;
    if (existing && existing.length > 5) {
      const keep = await askYesNo(`API key found (${existing.slice(0, 10)}...). Keep it?`, true);
      if (!keep) {
        const key = await askSecret('Enter your xAI API key');
        if (key) config.llm.xai = { ...config.llm.xai, api_key: key };
      }
    } else {
      const configureNow = await askYesNo('Add your xAI API key now?', true);
      if (configureNow) {
        const key = await askSecret('Enter your xAI API key (from console.x.ai)');
        if (key) {
          config.llm.xai = { ...config.llm.xai, api_key: key };
        } else {
          printWarn('No API key set. JARVIS won\'t work without one.');
        }
      } else {
    ...
    const currentModel = config.llm.xai?.model ?? 'grok-4-0709';
    const xaiModels = [
      { label: 'Grok 4', value: 'grok-4-0709', description: 'Latest flagship Grok model' },
      { label: 'Grok Code Fast 1', value: 'grok-code-fast-1', description: 'Fast coding-focused Grok model' },
      { label: 'Grok 3', value: 'grok-3', description: 'Balanced previous-generation Grok model' },
      { label: 'Grok 3 Mini', value: 'grok-3-mini', description: 'Smaller, lower-latency Grok model' },
      { label: 'Custom', value: 'custom', description: 'Enter model name manually' },
    ];
    const isPreset = xaiModels.some(m => m.value === currentModel);
  ...
  if (setupFallbacks) {
    for (const fb of config.llm.fallback) {
      ...
      } else if (fb === 'xai' && (!config.llm.xai?.api_key || config.llm.xai.api_key === '')) {
        const configureProvider = await askYesNo('Configure xAI as a fallback provider?', false);
        if (configureProvider) {
