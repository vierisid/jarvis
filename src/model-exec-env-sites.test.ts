...
describe('model-directed spawns strip the daemon secrets and keep the user env (#514)', () => {
  ...
  test('CONTROL: a spawn that inherits sees every canary, through both spawn APIs', async () => {
    ...
    for (const name of ['control-bun', 'control-node']) {
      ...
      // The markers come from modelExecEnv(), not from the harness -- nor from
      // a developer running this suite in a marked shell.
      expect({ name, marker: env![MODEL_EXEC_MARKER_ENV] }).toEqual({ name, marker: undefined });
      expect({ name, flag: env![MODEL_EXEC_ENV_KEY_FLAG] }).toEqual({ name, flag: undefined });
    }
  }, 45_000);
