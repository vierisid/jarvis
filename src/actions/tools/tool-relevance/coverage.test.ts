...
import { outsideReach, isFloorEligible, type OutsideReach } from './authority-classes.ts';
...
const EXPECTED: Record<string, OutsideReach> = {
  ...
  // inert -- status or locally generated facts only
  write_file: 'inert',
  set_clipboard: 'inert',
...
describe('daemon-registered tool classification', () => {
  const EXPECTED_RUNTIME: Record<string, { category: string; reach: OutsideReach }> = {
    ...
    commitments: { category: 'tasks', reach: 'replay' },
    manage_goals: { category: 'goals', reach: 'replay' },
    research_queue: { category: 'productivity', reach: 'replay' },
