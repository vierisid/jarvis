...
const EXEMPT: Record<string, Exemption> = {
  ...
  'actions/app-control/sidecar-launcher.ts': {
    reason:
      ...
      '`cmd.exe /C echo %USERPROFILE%`, which needs the WSL interop variables (WSL_INTEROP, WSLENV). The base ' +
      'allowlist drops those; since #519 a spawn can add them back as sanitizedEnv extras, as ' +
      'actions/terminal/wsl-bridge.ts does (not exempt). This probe keeps the inherited env here: its env is ' +
      'decided together with launchSidecar\'s in #514.',
