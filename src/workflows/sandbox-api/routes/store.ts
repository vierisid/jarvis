...
export const getStoreEntryRoute: RouteHandler = async (req: RouteContext) => {
  const url = new URL(req.url);
  const key = url.searchParams.get("key");
  if (!key) return err("missing 'key' query param", 400);
  try {
    const entry = getStoreEntry(req.claims.projectId, key);
...
export const putStoreEntryRoute: RouteHandler = async (req: RouteContext) => {
  ...
  try {
    const entry = putStoreEntry(req.claims.projectId, body.key, body.value);
...
export const deleteStoreEntryRoute: RouteHandler = async (req: RouteContext) => {
  const url = new URL(req.url);
  const key = url.searchParams.get("key");
  if (!key) return err("missing 'key' query param", 400);
  try {
    deleteStoreEntry(req.claims.projectId, key);
