import { combineDecisions, type AuthorityEngine } from '../../authority/engine';
  ...
  async invoke(input: EffectInvocation): Promise<EffectReply> {
    ...
    const policy = () => {
      ...
      const decision = combineDecisions(categories.map((actionCategory) => authority.checkAuthority({
        agentId: `workflow:${record.runId}`, agentRoleId: 'workflow-default',
        agentAuthorityLevel: 0, toolName: input.toolName, toolCategory: input.toolCategory,
        actionCategory, temporaryGrants: new Map() })));
